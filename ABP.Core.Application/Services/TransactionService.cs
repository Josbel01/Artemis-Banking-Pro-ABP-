...
"""
            ...
            {
                _logger.LogWarning(ex, "Credit card payment completed but card owner email notification failed.");
            }

            // Send email to origin account owner if different from card owner
            try
            {
                if (originAccount.ClientId != creditCard.ClientId)
                {
                    var accountOwner = await _accountService.GetUserById(originAccount.ClientId);
                    if (accountOwner != null)
                    {
                        ...
                        await _emailService.SendAsync(new ABP.Core.Application.Dtos.Email.EmailRequestDto
                        {
                            ...
                            Subject = $"Pago a tarjeta {lastFourCard} desde tu cuenta",
                            HtmlBody = $"""
<!DOCTYPE html>
<html><head><meta charset="utf-8"></head>
<body style="margin:0;padding:0;background-color:#f1f5f9;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;">
...
<h1 style="color:#fff;margin:0;font-size:20px;">&#128179; D&#233;bito en tu Cuenta</h1>
</div>
<div style="padding:30px;">
<p style="color:#334155;font-size:15px;margin:0 0 18px;">Hola <strong>{accountOwner.FirstName}</strong>,</p>
<p style="color:#334155;font-size:15px;margin:0 0 24px;">Se ha debitado dinero de tu cuenta de ahorro para realizar un pago a una tarjeta de cr&#233;dito.</p>
<table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
<tr><td style="padding:10px 14px;background:#f8fafc;color:#64748b;font-size:13px;font-weight:600;border-radius:6px 0 0 6px;">Monto debitado</td><td style="padding:10px 14px;background:#f8fafc;font-size:18px;font-weight:700;color:#dc2626;border-radius:0 6px 6px 0;">RD${effectiveAmount:N2}</td></tr>
<tr><td style="padding:10px 14px;color:#64748b;font-size:13px;font-weight:600;">Cuenta origen</td><td style="padding:10px 14px;font-size:15px;font-weight:700;color:#0b1f3a;">&#9679;&#9679;&#9679;&#9679; {lastFourAccount}</td></tr>
<tr><td style="padding:10px 14px;background:#f8fafc;color:#64748b;font-size:13px;font-weight:600;border-radius:6px 0 0 6px;">Tarjeta beneficiaria</td><td style="padding:10px 14px;background:#f8fafc;font-size:15px;color:#0b1f3a;border-radius:0 6px 6px 0;">&#9679;&#9679;&#9679;&#9679; &#9679;&#9679;&#9679;&#9679; &#9679;&#9679;&#9679;&#9679; {lastFourCard}</td></tr>
<tr><td style="padding:10px 14px;color:#64748b;font-size:13px;font-weight:600;">Fecha y hora</td><td style="padding:10px 14px;font-size:15px;color:#0b1f3a;">{DateTime.Now:dd/MM/yyyy HH:mm}</td></tr>
</table>
<div style="background:#fee2e2;border-left:4px solid #dc2626;padding:12px 16px;border-radius:0 6px 6px 0;margin-bottom:20px;">
<p style="color:#991b1b;font-size:13px;margin:0;">&#128683; Este pago fue realizado por un cajero. Si no reconoce esta operaci&#243;n, comun&#237;quese con la entidad bancaria.</p>
</div>
</div>
<div style="background:#f8fafc;padding:18px 30px;text-align:center;border-top:1px solid #e2e8f0;">
<p style="color:#94a3b8;font-size:11px;margin:0;">Artemis Banking Pro &mdash; Plataforma de Banca Digital ITLA</p>
...
"""
            ...
            {
                _logger.LogWarning(ex, "Credit card payment completed but account owner email notification failed.");
            }
